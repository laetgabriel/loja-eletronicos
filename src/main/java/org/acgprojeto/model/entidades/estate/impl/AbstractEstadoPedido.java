        ...
        try (PdfWriter writer = new PdfWriter(path);
            ...
            for (PedidoProdutoDTO pp : pedidoProdutos) {
                if (pp.getPedido().getIdPedido().equals(pedidoDTO.getIdPedido())) {
                    ...
                    valorTotalPedido = valorTotalPedido.add(produto.getPreco());
                }
            }
            ...
            List<ServicoDTO> servicos = servicoDAO.listarServicosPorPedido(pedidoDTO); // Método para buscar serviços do pedido
